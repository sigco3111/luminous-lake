...
- **6마리 어종**: 은송어(흔함) · 달빛피라미(밤) · 황혼우럭(황혼) · 금잉어(맑은 황혼) · 폭풍메기(비/폭풍) · 유령은어(전설, 맑은 한밤)
- **장력/진행 미니게임**: 줄을 감으면 장력 ↑·진행 ↑, 늦추면 장력 ↓·진행 ↓. 장력 1.0 = 줄 끊김, 진행 1.0 = 어획
- **환경이 입질을 좌우**: 같은 시간대·날씨·바람에 따라 어종 가중치·대기 시간·물보라 크기가 자연스럽게 변화
- **도감**: 잡은 어종은 자동으로 등록되고 종별 최장 크기·마리 수가 누적
- 모든 시뮬레이션은 `src/sim/fishing.js`의 순수 모듈(결정론적 RNG, delta-time)이라 단위 테스트로 검증 가능
...
│   ├── sim/                # 시뮬레이션 코어 (RNG, 파동, 날씨, 시간, 동물, 낚시)
│   └── world/              # 렌더링: 호수, 어선, 숲, 하늘, 동물 뷰, 낚시 뷰 등
├── test/                   # node --test 단위 테스트 + Playwright E2E (낚시·도감 포함)
